WITH installs AS (
    SELECT
        date_trunc('day', p_event_date) AS day,
        count(*) AS installs
    FROM
        delta.growth_adjust_odp.adjust_install_v3
    WHERE
        app_install_country = 'PL'
        AND app_install_event_time >= date('2024-08-15')
    GROUP BY
        1
),
signups AS (
    SELECT
        date_trunc('day', user_created_at) AS day,
    ...
    WHERE
        user_created_at >= DATE('2024-08-15')
...
new_customers AS (
    ...
    WHERE
        ...
        AND date(order_started_local_at) >= date '2024-08-15'
        AND o.order_is_first_delivered_order
    GROUP BY
        1
)
SELECT
    date(COALESCE(i.day, s.day, n.day)) AS day,
