WITH installs AS (
    SELECT date_trunc('month', p_event_date) AS month,
           count(*) AS installs
...
signups AS (
    SELECT date_trunc('month', user_created_at) AS month,
...
new_customers AS (
    SELECT date_trunc('month', order_started_local_at) AS month,
...
SELECT DATE (COALESCE(i.month, s.month, n.month)) AS month,
...
FROM installs i
JOIN signups s ON i.month = s.month
JOIN new_customers n ON COALESCE(i.month, s.month) = n.month
ORDER BY month DESC